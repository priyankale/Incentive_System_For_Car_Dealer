...
class AddCompanyComponent extends Component {
    ...
    saveOrUpdateCompany = (e) => {
        ...
        this.props.history.push("/add-company/_ccadd");
        if (this.state.id === '_ccadd') {
            CarDealerService.createCompany(company).then(res => {
                this.props.history.push('/companys');
            });
        } else {
            CarDealerService.updateCompany(company, this.state.id).then(res => {
                this.props.history.push('/companys');
            });
        }
    }
    ...
    componentDidMount() {
        ...
        if (this.state.id === '_ccadd') {
            ...
            CarDealerService.getCompanyById(this.state.id).then((res) => {
